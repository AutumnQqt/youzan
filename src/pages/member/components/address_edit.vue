<template>
  <div style="min-height: 597px;" class="container">
        ...
        <div class="block-item">
          ...
          <div class="select-group">
            <select class="js-province-selector" v-model="provinceValue">
              ...
              <option :value="province.value" v-for="province in provinceData">{{province.label}}</option>
            </select>
            <select class="js-city-selector"  v-model="cityValue">
              <option value="-1">选择城市</option>
              <option :value="city.value" v-for="city in cityData">{{city.label}}</option>
            </select>
            <select class="js-county-selector" name="area_code" data-code="440402" v-model="districtValue"> 
              <option value="-1">选择地区</option>
              <option :value="district.value" v-for="district in districtData">{{district.label}}</option>
            </select>
          </div>
        </div>
        <div class="block-item">
...
<script src="./address_edit.js"></script>
